...
/*
 ...
 1. return how many knights walked.
 
 */
int touring(struct knight dummy, int step) {
	
	int availableToGo = FALSE;
	
#ifdef CONSOLE
	int i, j;
	if (totalStep != 0) {
		printf("\n>> Step : %d, dummy now : {%d, %d} \n>> ", step-1, dummy.x+1, dummy.y+1);
	...
	if (ABLE_ULL(dummy) && BORD_ULL(dummy)!=STUCKED
		&& BORD_ULL(dummy)==EMPTY && availableToGo == FALSE) {
		
		RECORD_PATH(dummy);
		MOVE_ULL(dummy);
		if ((availableToGo=touring(dummy, step))==FALSE){
			RSET_ULL(dummy);
		}
		else availableToGo = TRUE;
	}
	if (ABLE_UUL(dummy) && BORD_UUL(dummy)!=STUCKED
		&& BORD_UUL(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_UUR(dummy) && BORD_UUR(dummy)!=STUCKED
		&& BORD_UUR(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_URR(dummy) && BORD_URR(dummy)!=STUCKED
		&& BORD_URR(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_DRR(dummy) && BORD_DRR(dummy)!=STUCKED
		&& BORD_DRR(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_DDR(dummy) && BORD_DDR(dummy)!=STUCKED
		&& BORD_DDR(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_DDL(dummy) && BORD_DDL(dummy)!=STUCKED
		&& BORD_DDL(dummy)==EMPTY && availableToGo == FALSE) {
	...
	if (ABLE_DLL(dummy) && BORD_DLL(dummy)!=STUCKED
		&& BORD_DLL(dummy)==EMPTY && availableToGo == FALSE) {
